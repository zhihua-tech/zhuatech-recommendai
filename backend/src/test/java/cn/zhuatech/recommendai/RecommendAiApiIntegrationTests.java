 ...
 */
@SpringBootTest @AutoConfigureMockMvc class RecommendAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanRankRecommendation()throws Exception{mvc.perform(post("/api/ai/recommend/rank").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"userSegment\":\"新客-家居\",\"candidateId\":\"SKU-88\",\"affinityScore\":92,\"businessPriority\":70,\"inventoryAvailability\":85,\"marginRate\":0.35,\"recentlyPurchased\":false,\"restrictedCategory\":false}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("RECOMMEND"));}}
